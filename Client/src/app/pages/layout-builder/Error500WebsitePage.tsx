...
const Error500WebsitePage: React.FC = () => {
    ...
    const getError500 = async (page: number = 1) => {
        try {
            if (auth && auth.api_token) {
                setLoading(true);
                const response = await fetch(`http://localhost:5000/api/get-error500website?page=${page}&limit=${itemsPerPage}`, {
                    headers: {
    ...
    const handlePageClick = (selectedPage: { selected: number }) => {
        ...
        getError500(selectedPageNumber);
    };
    ...
    const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (searchTerm === '') {
            setFilterError500(error500);
        } else {
            const results = error500.filter(item =>
                item.website_id.toString().includes(searchTerm) ||
                item.website_url.toLowerCase().includes(searchTerm) ||
                item.error_message.toString().includes(searchTerm) ||
